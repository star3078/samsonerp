...
using SamsonErp.Migrations.SeedData;
...
namespace SamsonErp.Tests
{
    public abstract class SamsonErpTestBase : AbpIntegratedTestBase<SamsonErpTestModule>
    {
        ...
        protected void UsingDbContext(Action<SamsonErpDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SamsonErpDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SamsonErpDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SamsonErpDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SamsonErpDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SamsonErpDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SamsonErpDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SamsonErpDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SamsonErpDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SamsonErpDbContext, Task<T>> func)
        {
            T result;
